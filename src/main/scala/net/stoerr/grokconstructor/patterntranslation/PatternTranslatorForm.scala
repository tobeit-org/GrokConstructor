...
case class PatternTranslatorForm(request: HttpServletRequest) extends WebForm {
  ...
  val format: InputText = InputText("format")

  def patternEntry: Elem = format.inputText("The log4j format (e.g. %d{ISO8601} %-14.14c{1}- %m%n) for which you want to create a grok pattern:", 180, 1)

  /** Used for output only. */
  val result: InputText = InputText("result")
